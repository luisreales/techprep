using TechPrep.Core.Enums;

namespace TechPrep.Application.DTOs.Sessions;

public record SessionTemplateCreateDto
{
    public string Name { get; init; } = default!;
    public string Mode { get; init; } = default!; // "study" or "interview"
    public List<int> Topics { get; init; } = new();
    public List<string> Levels { get; init; } = new(); // ["basic", "medium", "difficult"]
    public bool RandomOrder { get; init; } = true;
    public int? TimeLimitMin { get; init; }
    public int ThresholdWritten { get; init; } = 80;
    public QuestionsConfigDto Questions { get; init; } = new();
    public ChallengesConfigDto Challenges { get; init; } = new();
    public string Status { get; init; } = "draft"; // "draft" or "published"
}

public record QuestionsConfigDto
{
    public string Selection { get; init; } = "auto"; // "auto" or "manual"
    public AutoQuestionsConfigDto? Auto { get; init; }
    public List<string> ManualIds { get; init; } = new();
}

public record AutoQuestionsConfigDto
{
    public int Single { get; init; }
    public int Multi { get; init; }
    public int Written { get; init; }
    public Dictionary<string, int> ByLevel { get; init; } = new();
}

public record ChallengesConfigDto
{
    public string Selection { get; init; } = "auto"; // "auto" or "manual"
    public AutoChallengesConfigDto? Auto { get; init; }
    public List<int> ManualIds { get; init; } = new();
}

public record AutoChallengesConfigDto
{
    public int Total { get; init; }
    public Dictionary<string, int> ByLevel { get; init; } = new();
    public List<string> Languages { get; init; } = new();
}